...
if [[ "${2:-}" == "--flash" ]]; then
    while true; do
        if ~/.pico-sdk/picotool/2.2.0/picotool/picotool info >/dev/null 2>&1; then
            echo "Device found!"
            break
        fi
    ...
    ~/.pico-sdk/picotool/2.2.0/picotool/picotool load ".build/armv7em-none-none-eabi/release/$1.uf2"
    ~/.pico-sdk/picotool/2.2.0/picotool/picotool reboot
fi
